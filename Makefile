MAKEFLAGS  := -j 1
INS         = source/beamerthemescpo.ins
...
DOC_SRC     = doc/scpotheme.dtx
DOC_PDF     = doc/scpotheme.pdf

CTAN_CONTENT = README.md $(INS) $(PACKAGE_SRC) $(DOC_SRC) $(DOC_PDF) $(DEMO_SRC) $(DEMO_PDF)

DESTDIR     ?= $(shell kpsewhich -var-value=TEXMFHOME)
INSTALL_DIR  = $(DESTDIR)/tex/latex/scpo
DOC_DIR      = $(DESTDIR)/doc/latex/scpo
...
ctan: $(CTAN_CONTENT) ctan-version
	@tar --transform "s@\(.*\)@scpo/\1@" -cf scpo-$(shell date "+%Y-%m-%d").tar.gz $(CTAN_CONTENT)
